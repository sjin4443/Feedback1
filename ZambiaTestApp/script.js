...
function showAtomsCard() {
  ...
  // Always activate dropdown menu when entering atoms page
  ...
  // Completely clear image container when entering
  const atomsImageContainer = document.getElementById('atomsImageContainer');
  if (atomsImageContainer) {
    atomsImageContainer.innerHTML = '';
  }

  // Also refresh TOC menu to avoid accidental auto-loading
  showTOC('eyes');  // Default tab selection to 'eyes' when entering
}
...
if (tocToggleBtn) {
  tocToggleBtn.addEventListener('click', () => {
    ...
    if (dropdown) {
      ...
      dropdown.classList.remove('hidden'); // ← remove hidden class so it can be shown
    }
  });
    ...
    tocTabButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        tocTabButtons.forEach(b => b.classList.remove('active')); // remove red
        btn.classList.add('active'); // set clicked to red
      });
    });
  ...
  if (closeTOCBtn) {
    closeTOCBtn.addEventListener('click', () => {
      ...
      if (dropdown) {
        ...
        dropdown.classList.add('hidden');  // ADD THIS LINE
      }
    });
  }
...
pupilCards.forEach(card => {
  ...
  card.addEventListener('click', () => {
    switch (title) {
      case 'RAPD Test':
        showPage('rapdTestPage');
        break;
      case 'Pupil Exam':
        showPage('pupilExamPage');
        break;
      case 'Pupil Exam PEC':
...
function showTOC(type) {
  ...
  const eyes = [
    ...
    'Fundal Reflex', 'Fundus', 'Glaucoma', 'How to Check for Eyeglasses', 'How to Use',
    'Lens', 'Pupil', 'Red Eye', 'Summary', 'Vision Loss'
  ];

  const ears = ['Anatomy', 'Childhood Hearing Development', 'Ear Drum', 'External Ear: Pinna'];

  const items = type === 'eyes' ? eyes.sort() : ears.sort();

  // Always blank image area when switching tabs
const atomsImageContainer = document.getElementById('atomsImageContainer');
